        	...
        	if (!!lyr.lyric) {
                ...
                if(line != ""){
					for (var i = 0; i < line.length; i++) {//遍历歌词数组
						...
						for (var j = 0; j < time.length; j++) {//遍历时间数组
							// log('time[j]', time[j].slice(1, -1))
							...
							// log('timeArr', timeArr, parseInt(t[0], 10) * 60, parseFloat(t[1]))
...
var renderLyric = function() {
	// log('lyricArr', lyricArr)
	var lyrLi = "";
